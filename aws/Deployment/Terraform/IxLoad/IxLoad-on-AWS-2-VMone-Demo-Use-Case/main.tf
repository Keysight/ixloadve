...
resource "aws_network_interface" "KeysightClientEth0" {
...
resource "aws_instance" "KeysightClient" {
	disable_api_termination = local.INSTANCExDISABLExAPIxTERMINATION
	instance_initiated_shutdown_behavior = local.INSTANCExINSTANCExINITIATEDxSHUTDOWNxBEHAVIOR
	...
	network_interface {
		network_interface_id = aws_network_interface.KeysightClientEth0.id
		device_index =  "0"
	}
	ebs_block_device {
		device_name = local.INSTANCExBLOCKxDEVICExNAME
...
resource "aws_eip" "MgmtElasticIp" {
	vpc = true
	network_interface = aws_network_interface.KeysightClientEth0.id
	depends_on = [
		aws_internet_gateway.InternetGw,
		aws_instance.KeysightClient
	]
}
